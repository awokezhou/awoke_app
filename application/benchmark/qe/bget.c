...
/* #define EocTrace   1 */ /* print each memory allocation and free */
...
#ifdef EocTrace
		    eocLog_debug("bget: %lu->%lu total=%lu %lu/%lu",
...
#ifdef EocTrace
    eocLog_debug("brel: %ld total=%lu %lu/%lu",
...
void secondary_bpool(void *buf)
{
   /*
    * This entry point is used when the bget pools have already been 
    * initialized by the first application to initialize DB and the
