...
struct mg_connection *mg_connect_opt(struct mg_mgr *mgr, const char *address,
                                     MG_CB(mg_event_handler_t callback,
                                           void *user_data),
                                     struct mg_connect_opts opts) {
  log_debug("mg_connect_opt address %s", address);                              
  ...
  log_debug("will parse_address %s", address);
  ...
  log_debug("parse_address %s", address);
  ...
  if (rc == 0) {
#if MG_ENABLE_ASYNC_RESOLVER
    log_debug("MG_ENABLE_ASYNC_RESOLVER");
    /*
	...
	log_debug("will mg_resolve_async_opt()");
	...
	log_debug("mg_resolve_async_opt() ok");
    nc->priv_2 = dns_conn;
    nc->flags |= MG_F_RESOLVING;
	log_debug("nc->flags:0x%lx", nc->flags);
    return nc;
#else
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now) {
  log_debug("fd:%d, fd_flags:%d, nc_flags:0x%lx", nc->sock, fd_flags, nc->flags);
  int worth_logging =
...
time_t mg_socket_if_poll(struct mg_iface *iface, int timeout_ms) {
  ...
  for (nc = mgr->active_connections; nc != NULL; nc = tmp) {
	...
	if (FD_ISSET(nc->sock, &read_set))
		log_debug("sock:%d, add to read_set ok", nc->sock);
	if (FD_ISSET(nc->sock, &write_set))
		log_debug("sock:%d, add to write_set ok", nc->sock);
...
struct mg_connection *mg_connect_http_base(
  ...
  log_debug("host:");				   
  fwrite(host->p, 1, host->len, stdout);
  ...
  log_debug("url:%s, use_ssl:%d, conn_addr:%s", url, use_ssl, conn_addr);
  ...
  if ((nc = mg_connect_opt(mgr, conn_addr, MG_CB(ev_handler, user_data),
                           opts)) != NULL) {
    log_debug("mg_connect_opt return not NULL");
    mg_set_protocol_http_websocket(nc);
  }
  log_debug("mg_connect_opt ok, conn_addr %s", conn_addr);
out:
...
struct mg_connection *mg_connect_http_opt(
  ...
  log_debug("method:%s", post_data);

  mg_printf(nc, "%s %.*s HTTP/1.1\r\nHost: %.*s\r\nContent-Length: %" SIZE_T_FMT
                "\r\n%.*s%s\r\n%s",
            post_data, (int) path.len, path.p,
...
static void mg_resolve_async_eh(struct mg_connection *nc, int ev,
  ...
  log_debug("callback:mg_resolve_async_eh, ev:%d", ev);
...
int mg_resolve_async_opt(struct mg_mgr *mgr, const char *name, int query,
                         mg_resolve_callback_t cb, void *data,
                         struct mg_resolve_async_opts opts) {
  log_debug("mg_resolve_async_opt() in");
  struct mg_resolve_async_request *req;
  ...
  log_debug("awoke will mg_connect()");
