...
uint8_t *awoke_string_padding(uint8_t *s, char c, int align, int *rlen) 
{
	...
	log_debug("end-p %d, len %d", end-p, len);
...
int main(int argc, char *argv[])
{
	...
	//buff_full_with_byte_rand(test, array_size(test));
	...
	uint8_t *str_plain = "hello world";
	uint8_t plain[10] = {
		0x03, 0x05, 0x2d, 0x1c, 0xaa, 0xe3, 0xff, 0x23, 0x17, 0x89
	};
	uint8_t dec_plain[BUFF_LEN] = {0x0};
	uint8_t cipher[BUFF_LEN] = {0x0};
